    ...
    <div class="row">
      <section class="row-section1">
        <div class="container-fluid px-4">
          <div class="row pt-5">
            <h2 class="text-center m-0"><span>Gestionar menús pacientes</span></h2>
          </div>
          <div class="row">
                      ...
                      <thead class="bg-success">
                        ...
                        <th>Opciones</th>
                          ...
                          <template v-for="item in totalCantidadAlimentosLista">
                            <!-- <template v-if="item.fecha==listaPacientePlato.fecha&&item.tipoDePlato==listaPacientePlato.tipoDePlato">
                              <template v-for="item2 in alimentoPlato">
                                <template v-if="item2.idPlato==item.idPlato">
                                  <template v-for="item3 in alimentos">
                                    <template v-if="item3['.key']==item2.idAlimento"> -->
                                      ...
                                      <tr class="table-success">
                                        ...
                                        <td>
                                          ...
                                          <button  v-on:click="" class="btn btn-danger" type="button" name="button"> <span class="fa fa-trash-o"> </span> </button>
                                        </td>
                                      </tr>
                                    <!-- </template>
                                  </template>
  ...
  methods: {
    ...
    buscadorPlato(platosR) {
      ...
      return platosR.filter((item) => {
        return item.nombre.toUpperCase().match(plato.toUpperCase()) || item.tipoDeDieta.toUpperCase().match(plato.toUpperCase());
    ...
    asignarPaciente(item) {
      ...
      // this.pacientePlato.push(item);
      // for (var i = 0; i < this.pacientePlato.length; i++) {
      //   console.log(this.pacientePlato[i]);
      // }
    },
    asignarPlato(item) {
    ...
    listarAlimentos() {
      ...
      var arrayConteo = 0;
      for (var i = 0; i < this.pacientePlato.length; i++) {
        if (this.pacientePlato[i].fecha == this.listaPacientePlato.fecha && this.pacientePlato[i].tipoDePlato == this.listaPacientePlato.tipoDePlato) {
                ...
                if (this.alimentos[a]['.key'] == this.alimentoPlato[e].idAlimento) {
                  ...
                  if (conteo <= 1) {
                    ...
                    for (var b = 0; b < this.pacientePlato.length; b++) {
                      if (this.pacientePlato[b].fecha == this.listaPacientePlato.fecha && this.pacientePlato[b].tipoDePlato == this.listaPacientePlato.tipoDePlato) {
    ...
    exportPdf() {
      ...
      doc.save('Lista de ingrdientes ' + this.listaPacientePlato.fecha + '.pdf');
    }
